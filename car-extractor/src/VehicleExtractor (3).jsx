...
async function jinaGet(url, target) {
  const headers = { Accept: "text/plain", "X-No-Cache": "true" };
...
async function fetchPage(url) {
  // Pierwsze żądanie sekwencyjnie — jeśli fail, rzucamy błąd od razu
  const main = await jinaGet(url, null);
  if (!main || main.length < 100) throw new Error("Pusta odpowiedź z Jina AI");

  // Dwa dodatkowe równolegle — celują w ukryte sekcje, błędy ignorujemy
  const [params, history] = await Promise.all([
    jinaGet(url, "[data-testid='ad-params'], .offer-params, [class*='Params'], [class*='SpecList'], [class*='technical']").catch(() => ""),
    jinaGet(url, "[data-testid='ad-history'], [class*='History'], [class*='Condition'], [class*='Stan']").catch(() => ""),
  ]);

  return [main, params, history]

...
// Real VINs always contain letters (positions 1,2,9,10,11 are alpha). Reject pure-digit strings.
const isValidVin = s => /^[A-HJ-NPR-Z0-9]{17}$/.test(s) && /[A-HJ-NPR-Z].*[A-HJ-NPR-Z].*[A-HJ-NPR-Z].*[A-HJ-NPR-Z]/.test(s);
const vinFromHtml = html => {
  const m = html.match(/[A-HJ-NPR-Z0-9]{17}/g);
  return m?.find(v => isValidVin(v)) ?? null;
};
...
function flattenParams(raw) {
  ...
  if (Array.isArray(raw)) {
    raw.forEach(item => {
      if (!item?.key) return;
      p[item.key] = item.value?.label ?? item.value?.key ?? item.normalizedValue ?? item.value;
    });
...
function parseOtomoto(html, url) {
  ...
  // 1. Target <script id="__NEXT_DATA__"> directly (Next.js standard)
  let nd = null;
  const ndScript = doc.querySelector("script#__NEXT_DATA__");
  if (ndScript) {
    try { nd = JSON.parse(ndScript.textContent); } catch {}
  }

  // 2. Fallback: scan non-src scripts for one that has advert data
  if (!nd) {
    for (const s of doc.querySelectorAll("script:not([src])")) {
      const t = s.textContent?.trim() ?? "";
      if (t.length < 200 || !t.includes("\"make\"")) continue;
      try {
        const start = t.indexOf("{");
        if (start === -1) continue;
        const c = JSON.parse(t.slice(start));
        if (c?.props || deepFind(c, "advert") || deepFind(c, "make")) { nd = c; break; }
      } catch {}
    }
  }

  // 3. Navigate to advert (otomoto Next.js: nd.props.pageProps.advert)
  const advert =
  ...
  // 4. Flatten params
  const params = flattenParams(advert?.params ?? deepFind(advert, "params"));
  const get = (...keys) => {
    for (const k of keys) {
      const v = params[k];
  ...
  const brand = clean(
    ...
    get("make", "brand") ?? ld?.brand?.name
  );
  const model = clean(
    advert?.model?.label ?? advert?.model ??
    get("model", "car_model") ?? ld?.model
  );
  ...
  return {
    brand, model, variant,
    year: toNum(advert?.year ?? get("year") ?? ld?.vehicleModelDate),
    price, currency,
    mileage: toNum(advert?.mileage ?? get("mileage", "odometer", "mileage_from_odometer") ?? ld?.mileageFromOdometer?.value),
    engineDisplacement: toNum(get("engine_capacity", "engineDisplacement") ?? ld?.engineDisplacement?.value),

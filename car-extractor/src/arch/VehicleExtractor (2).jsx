...
// Wysyłamy 3 równoległe żądania do r.jina.ai z różnymi X-Target-Selector,
// żeby wymusić pobranie ukrytych sekcji (specyfikacja, stan, historia).
// Wszystkie 3 lecą jednocześnie — łączny czas ≈ czas najwolniejszego.
async function fetchPage(url) {
  // Selektory celujące w konkretne sekcje otomoto/olx
  const targets = [
    null,                                    // 1. cała strona (tytuł, cena, zdjęcia, opis)
    "main, [data-testid='ad-details'], article, .offer-content", // 2. szczegóły ogłoszenia
    "[data-testid='ad-params'], [data-testid='technical-params'], .offer-params, [class*='SpecList'], [class*='Params'], [class*='technical'], [class*='specif']", // 3. parametry techniczne
  ];

  const jinaFetch = (targetSelector) =>
  ...
  const [main, details, params] = await Promise.all(targets.map(jinaFetch));

  if (!main || main.length < 100) throw new Error("Pusta odpowiedź z Jina AI");

  // Łączymy wszystkie odpowiedzi — duplikaty w parserze nie szkodzą,
  // bo field() zwraca pierwsze trafienie, a Set() deduplikuje obrazki.
  return [main, details, params].filter(Boolean).join("\n\n---SECTION---\n\n");
}
...
function parseMd(md, url) {
  ...
  const brand = clean(field(md, "Marka pojazdu", "Marka", "Make") ?? tBrand);
  const model = clean(field(md, "Model pojazdu", "Model") ?? tModel);
  ...
  return {
    ...
    price, currency,
    mileage:
      fieldNum(md, "Przebieg", "Przebieg (km)", "Mileage") ??
      findNumUnit(md, "km\\b"),
    engineDisplacement:
    ...
    enginePower:
      ...
      findNumUnit(md, "KM\\b"),
    enginePowerUnit: "KM",
    fuelType:      field(md, "Rodzaj paliwa", "Paliwo", "Fuel type"),
    transmission:  field(md, "Skrzynia biegów", "Skrzynia", "Transmission", "Gearbox"),
    drivetrain:    field(md, "Napęd", "Rodzaj napędu", "Drive"),
    bodyType:      field(md, "Typ nadwozia", "Nadwozie", "Body type"),
    color:         field(md, "Kolor", "Kolor nadwozia", "Color"),
    doors:         fieldNum(md, "Liczba drzwi", "Drzwi", "Doors"),
    seats:         fieldNum(md, "Liczba miejsc", "Miejsca", "Seats"),
    firstRegistration: cleanDate(
      field(md, "Data pierwszej rejestracji w historii pojazdu", "Data pierwszej rejestracji", "Pierwsza rejestracja")
    ),
    licensePlate: field(md, "Numer rejestracyjny pojazdu", "Numer rejestracyjny", "License plate"),
    vin: null, // Otomoto nie udostępnia VIN publicznie
    location:
      field(md, "Lokalizacja", "Miasto", "Location", "City") ??
      (() => {
        const m = md.match(/\[([^\]]+)\]\(https?:\/\/(?:maps|www\.google\.com\/maps)/i);
        return m ? clean(m[1]) : null;
      })(),
    seller: field(md, "Sprzedający", "Informacje o sprzedającym", "Seller"),

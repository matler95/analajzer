...
/* ─── FETCH ──────────────────────────────────────────────── */
// allorigins.win zwraca surowy HTML przez GET — szybkie, bez klucza.
// Cały HTML zawiera <script id="__NEXT_DATA__"> ze WSZYSTKIMI danymi
// ogłoszenia (w tym ukryte sekcje), więc jeden request wystarczy.
async function fetchPage(url) {
  ...
  if (!html || html.length < 500) throw new Error("Pusta odpowiedź z proxy");
  return html;
}

/* ─── PARSER ─────────────────────────────────────────────── */
// Strategia: HTML z allorigins zawiera <script id="__NEXT_DATA__"> 
// ze wszystkimi danymi ogłoszenia w JSON — w tym parametry z ukrytych sekcji.
// Fallback: regex na surowym HTML gdy __NEXT_DATA__ nie ma danej wartości.
...
const clean = v => v != null ? String(v).trim().replace(/\s+/g, " ") || null : null;
const cleanDate = v => {
  if (!v) return null;
...
// Spłaszcza tablicę params [{key, value:{label}}] → {key: "label"}
function flatParams(raw) {
  const p = {};
  if (!raw) return p;
  if (Array.isArray(raw)) {
    for (const item of raw) {
      if (!item?.key) continue;
      const v = unwrap(item?.value?.label) ?? unwrap(item?.normalizedValue) ?? unwrap(item?.value);
      if (v && v.trim()) p[item.key] = v.trim();
    }
...
// Regex fallback na surowym HTML
function rxFind(html, patterns) {
  for (const re of patterns) {
    const m = html.match(re);
    if (m?.[1]) return clean(m[1].replace(/\\u([\da-fA-F]{4})/g, (_, h) => String.fromCharCode(parseInt(h, 16))));
  }
  return null;
}
...
function normalizeImages(raw) {
  ...
  return arr.map(p => p?.url ?? p?.large ?? p?.medium ?? (typeof p === "string" ? p : null))
    .filter(u => u && /^https?:\/\//i.test(u));
}

function detectPortal(url) {
  if (/otomoto\.pl/i.test(url)) return "otomoto.pl";
  if (/olx\.pl/i.test(url)) return "olx.pl";
  return "";
}
...
function parseHTML(html, url) {
  ...
  // 1. Wyciągnij __NEXT_DATA__
  const ndMatch = html.match(/<script[^>]*id=["']__NEXT_DATA__["'][^>]*>([\s\S]*?)<\/script>/i);
  const nd = safeJson(ndMatch?.[1]);

  // 2. Znajdź obiekt ogłoszenia
  const pp = nd?.props?.pageProps ?? nd?.pageProps ?? {};
  const advert = pp?.advert ?? pp?.ad ?? pp?.listing ?? deepFind(pp, "advert") ?? deepFind(nd, "advert") ?? {};
  const params = flatParams(advert?.params ?? deepFind(advert, "params") ?? deepFind(pp, "params"));

  // 3. JSON-LD jako dodatkowe źródło
  let ld = null;
  ...
  const ogTitle = html.match(/property=["']og:title["'][^>]*content=["']([^"']{3,120})["']/i)?.[1];
  const h1 = html.match(/<h1[^>]*>([^<]{2,100})<\/h1>/i)?.[1];
  const title = clean(h1 ?? ogTitle ?? titleMatch?.[1])?.split("|")[0]?.split(" - ")[0];

  // Marka i model z tytułu jako ostateczny fallback
  const titleTokens = (title ?? "").split(/\s+/).filter(t => !/^[12]\d{3}$/.test(t) && t.length > 1);

  const brand = clean(
    unwrap(advert?.make) ?? getP(params, "make", "brand", "car_brand") ??
    rxFind(html, [/"make"\s*:\s*\{[^}]*"label"\s*:\s*"([^"]{1,40})"/i, /"car_brand"\s*:\s*"([^"]{1,40})"/i]) ??
    ld?.brand?.name ?? titleTokens[0]
  );
  const model = clean(
    unwrap(advert?.model) ?? getP(params, "model", "car_model") ??
    rxFind(html, [/"model"\s*:\s*\{[^}]*"label"\s*:\s*"([^"]{1,40})"/i, /"car_model"\s*:\s*"([^"]{1,40})"/i]) ??
    ld?.model ?? titleTokens[1]
  );

  let variant = null;
  if (title && brand && model) {
    const v = title.replace(new RegExp(brand.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),"i"),"")
  ...
  // Cena
  const priceUnits = advert?.price?.amount?.units ?? advert?.price?.value ?? pp?.price?.value;
  const price = toNum(
    priceUnits ?? getP(params, "price") ??
    rxFind(html, [/"amount"\s*:\s*\{[^}]*"units"\s*:\s*([\d]+)/i, /"price"\s*:\s*(\d[\d\s]*)/i]) ??
    ld?.offers?.price
  );
  const currency = clean(advert?.price?.currency ?? ld?.offers?.priceCurrency) ?? "PLN";

  // Parametry techniczne — params[] jest kompletna, zawiera ukryte sekcje
  const year = toNum(
    advert?.year ?? getP(params, "year", "production_year", "model_year") ??
    rxFind(html, [/"year"\s*:\s*(\d{4})/i, /"production_year"\s*:\s*"?(\d{4})/i]) ??
    ld?.vehicleModelDate
  );
  const mileage = toNum(
    advert?.mileage ?? getP(params, "mileage", "odometer", "mileage_from_odometer") ??
    rxFind(html, [/"mileage"\s*:\s*(\d+)/i, /"odometer"\s*:\s*(\d+)/i]) ??
    ld?.mileageFromOdometer?.value
  );
  const engineDisplacement = toNum(
    getP(params, "engine_capacity", "engineDisplacement") ??
    rxFind(html, [/"engine_capacity"\s*:\s*\{[^}]*"value"\s*:\s*([\d]+)/i, /"engine_capacity"\s*:\s*(\d+)/i]) ??
    ld?.engineDisplacement?.value
  );
  const enginePower = toNum(
    getP(params, "engine_power", "power") ??
    rxFind(html, [/"engine_power"\s*:\s*\{[^}]*"value"\s*:\s*([\d]+)/i, /"engine_power"\s*:\s*(\d+)/i]) ??
    ld?.vehicleEngine?.enginePower?.value
  );

  // Paliwo, skrzynia itd — label z params jest już po polsku
  const fuelType    = getP(params, "fuel_type", "fuelType") ?? ld?.fuelType ?? null;
  const transmission= getP(params, "gearbox", "transmission") ?? ld?.vehicleTransmission ?? null;
  const drivetrain  = getP(params, "drive", "driveWheels") ?? null;
  const bodyType    = getP(params, "body_type", "bodyStyle", "car_type") ?? ld?.bodyType ?? null;
  const color       = getP(params, "color", "colour") ?? null;
  const doors       = toNum(getP(params, "doors_count", "doors"));
  const seats       = toNum(getP(params, "nr_seats", "seats"));

  // Stan i historia — też są w params
  const condition   = getP(params, "condition", "stan", "state") ?? null;
  const damaged     = getP(params, "damaged", "accident_free", "bezwypadkowy") ?? null;
  const registered  = getP(params, "registered", "rejestracja") ?? null;
  const rhd         = getP(params, "right_hand_drive") ?? null;
  const imported    = getP(params, "imported", "import") ?? null;
  const tuning      = getP(params, "tuning") ?? null;

  // Data rejestracji, lokalizacja, sprzedający
  const firstRegistration = cleanDate(
    advert?.firstRegistrationDate ?? getP(params, "first_registration", "registration_date", "first_registration_year") ??
    rxFind(html, [/"firstRegistrationDate"\s*:\s*"([^"]{4,20})"/i])
  );
  const licensePlate = clean(getP(params, "plate_number", "registration_number")) ?? null;
  const location = clean(
    advert?.location?.city?.name ?? advert?.location?.name ??
    deepFind(advert, "cityName") ?? deepFind(pp, "cityName") ??
    rxFind(html, [/"cityName"\s*:\s*"([^"]{2,60})"/i, /"city"\s*:\s*\{[^}]*"name"\s*:\s*"([^"]{2,60})"/i])
  );
  const seller = clean(
    advert?.advertiser?.name ?? advert?.seller?.name ??
    deepFind(advert, "sellerName") ??
    rxFind(html, [/"(?:sellerName|advertiserName)"\s*:\s*"([^"]{2,80})"/i])
  );

  // Opis i zdjęcia
  const description = clean(advert?.description ?? deepFind(pp, "description") ?? null);
  const images = normalizeImages(advert?.photos ?? advert?.images ?? deepFind(advert, "photos"));

  return {
    brand, model, variant, year, price, currency,
    mileage, engineDisplacement, enginePower, enginePowerUnit: "KM",
    fuelType, transmission, drivetrain, bodyType, color, doors, seats,
    firstRegistration, licensePlate,
    condition, damaged, registered, rhd, imported, tuning,
    vin: null,
...
const FIELDS = [
  ...
  { key: "damaged",            lbl: "Uszkodzony" },
  { key: "registered",         lbl: "Zarejestrowany" },
  { key: "imported",           lbl: "Import" },
];
  ...
  return (
    <div className="app">
      <header className="hdr">
        ...
        <div className="hdr-pill">BEZ AI · BEZ KLUCZA</div>
      </header>
      ...
      <div className="main">
        <div className="input-area">
          ...
          <div className="hint">
            ...
            <span className="hint-dot">·</span> Parser HTML + __NEXT_DATA__ — bez AI, bez klucza
          </div>
        </div>
        ...
        {loading && (
          <div className="loader">
            <div className="spin" />
            <div className="load-msg">▶ Pobieranie ogłoszenia…</div>
          </div>
        )}
        ...
        {data && (
          <div className="result">
            ...
            <div className="note">
              <strong>ℹ Jak działa</strong> — Strona pobierana jest przez{" "}
              <strong style={{color:"var(--amber)"}}>allorigins.win</strong> (CORS proxy),
              następnie parser odczytuje dane z osadzonego JSON (<code>__NEXT_DATA__</code>)
              — w tym parametry z ukrytych sekcji specyfikacji i historii.

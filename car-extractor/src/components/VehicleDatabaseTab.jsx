...
import { PRICE_CHANGE_TTL_MS } from "../utils/constants.js"; // FIX #9
...
function PriceChangeBadge({ priceDiff, compact = false }) {
  ...
  const label  = isDrop ? `▼ ${fmt(abs)} PLN` : `▲ ${fmt(abs)} PLN`;
  return (
    <span
...
// FIX #18: NoteWidget wrapper in expanded list view needs stopPropagation
// so clicking inside the note section doesn't toggle the card closed.
function NoteWidget({ vehicleId, note, onSave }) {
  ...
  if(!open) return (
    <button type="button" className={`note-toggle ${note?"note-toggle--has":""}`}
      onClick={e=>{e.stopPropagation();setDraft(note??"");setOpen(true);}}
      title={note?`Notatka: ${note}`:"Dodaj notatkę"}>
      {note?"📝":"✎"}
    </button>
  );
  // FIX #18: The editor div itself also stops propagation so any click inside
  // (textarea, buttons) doesn't bubble up to the card header toggle.
  return (
  ...
  if (viewMode==="compact") return (
    <div className={["vdbc-compact",selected?"vdbc-compact--selected":"",pendingDelete?"vdbc--pending-delete":"",viewed?"vdbc-compact--viewed":"",isNew?"vdbc-compact--new":"",isArchived?"vdbc-compact--archived":"",priceChanged?"vdbc-compact--price-changed":""].filter(Boolean).join(" ")}>
      ...
      <div className="vdbc-compact-price">
        {s.price&&(<><span className="vdbc-compact-price-val">{fmt(s.price)}</span><span className="vdbc-compact-price-cur"> PLN</span>{priceChanged&&priceDiff!=null&&(<span className={`vdbc-diff ${priceDiff<0?"vdbc-diff--down":"vdbc-diff--up"}`} style={{fontSize:9,marginLeft:4}}>{priceDiff<0?"▼":"▲"}{fmt(Math.abs(priceDiff))}</span>)}</>)}
      </div>
    ...
    <div className={["vdbc",expanded?"vdbc--open":"",isArchived?"vdbc--archived":"",isNew?"vdbc--new":"",pendingDelete?"vdbc--pending-delete":"",selected?"vdbc--selected":"",viewed?"vdbc--viewed":"",priceChanged?"vdbc--price-changed":""].filter(Boolean).join(" ")}>
      ...
      {expanded&&(
        <div className="vdbc-body">
          {s.images?.length>1&&<div className="vdbc-imgstrip">{s.images.slice(0,8).map((u,i)=><img key={i} src={u} alt="" className="vdbc-strip-img" loading="eager"/>)}</div>}
          {(()=>{
          ...
          {priceHist.length > 0 && (
            <div className="vdbc-section">
              <div className="vdbc-section-hdr">
                Historia ceny
                {priceChanged && priceDiff != null && (
                  <span className={`vdbc-price-change-inline ${priceDiff < 0 ? "vdbc-price-change-inline--drop" : "vdbc-price-change-inline--rise"}`}>
              ...
              <div className="vdbc-price-chain">
                {[...priceHist, { price: s.price, recordedAt: null }].map((ph, i, arr) => (
                  <div key={i} className={`vdbc-chain-item ${i === arr.length - 1 ? "vdbc-chain-item--now" : ""}`}>
                    ...
                    <span className="vdbc-chain-date">{ph.recordedAt ? new Date(ph.recordedAt).toLocaleDateString("pl-PL") : "teraz"}</span>
                    {i < arr.length - 1 && (() => {
                      const next = arr[i + 1].price;
                      if (next == null || ph.price == null) return null;
                      const diff = next - ph.price;
                      if (Math.abs(diff) < 100) return null;
                      return <span className={`vdbc-chain-arrow ${diff < 0 ? "vdbc-chain-arrow--drop" : "vdbc-chain-arrow--rise"}`} title={`${diff < 0 ? "Spadek" : "Wzrost"} o ${fmt(Math.abs(diff))} PLN`}>{diff < 0 ? "▼" : "▲"}</span>;
                    })()}
                  </div>
                ))}
              </div>
              <div style={{ marginTop: 8 }}><PriceSparkline priceHistory={priceHist} currentPrice={s.price} /></div>
            </div>
          )}
          ...
          {/* FIX #18: stopPropagation on the note section wrapper prevents
              clicking it from toggling the card closed in list view */}
          <div className="vdbc-section" style={{background:"var(--card)"}} onClick={e=>e.stopPropagation()}>
...
// FIX #4: Accept isJobRunning as prop (was hardcoded false in FilterGroup call).
    ...
    <div className={`vdb-group ${collapsed?"vdb-group--collapsed":""}`}>
      <div className="vdb-group-hdr vdb-group-hdr--sticky" onClick={onToggle}>
        ...
        <div className="vdb-group-badges">
          ...
          {priceChangeCount>0&&<span className="vdb-gbadge vdb-gbadge--price" title="Zmiana ceny wykryta podczas ostatniego skanu">{priceChangeCount} zm. ceny</span>}
        </div>
        {!collapsed&&(
        ...
        {filter&&!collapsed&&(
          <button type="button" className="vdb-group-scan-btn" onClick={e=>{e.stopPropagation();onRunScan?.(filter);}}
            disabled={isJobRunning} title={`Skanuj „${filter.name}"`}>▶</button>
        )}
...
/* ══════════════ MAIN EXPORT ══════════════ */
// FIX #4: Accept isJobRunning prop.
// FIX #6: Accept refreshRef to expose loadVehicles without remounting.
export default function VehicleDatabaseTab({ me, onOpenItem, filters=[], onRunFilter, isJobRunning = false, refreshRef }) {
  const [vehicles,setVehicles]=useState([]);
  const [loading,setLoading]=useState(false);
  const [refreshing,setRefreshing]=useState(false); // FIX #11
  ...
  const vehiclesLenRef=useRef(0); // FIX #11: track current count without stale closure
  ...
  // FIX #11: Show a subtle "refreshing" banner (not full spinner) when we
  // already have data.  vdb-loading full spinner only shows on the first load.
  const loadVehicles=useCallback(async()=>{
    if(!me)return;
    if(vehiclesLenRef.current > 0){
      setRefreshing(true);
    } else {
      setLoading(true);
    }
    try{
      ...
      if(r.ok){
        ...
        setVehicles(rows);
        vehiclesLenRef.current = rows.length;
      }
    }finally{
      setLoading(false);
      setRefreshing(false);
  ...
  // FIX #6: Expose loadVehicles to parent via ref (no more key={dbRefreshKey}).
  useEffect(()=>{
    if(refreshRef) refreshRef.current = loadVehicles;
    return ()=>{ if(refreshRef) refreshRef.current = null; };
  },[refreshRef, loadVehicles]);
  ...
  const filtered=useMemo(()=>{
    let l=vehicles.filter(v=>!new Set(undoStack.map(u=>u.id)).has(v.id));
    ...
    return l;
  },[vehicles,filterStatus,searchQuery,undoStack]);
  ...
  // FIX #19: O(n) single-pass grouping instead of O(n×filters).
  // Old code iterated all vehicles for every filter group membership check.
  // New code builds all groups in one pass over the filtered list.
  const grouped = useMemo(() => {
    const g = new Map();
    const ensure = key => { if (!g.has(key)) g.set(key, []); };

    for (const v of filtered) {
      const src = v.snapshot_json?.__source;
      if (src === "manual") {
        ensure("__manual");
        g.get("__manual").push(v);
      } else {
        const filterNames = v.snapshot_json?.__filterNames ||
          [v.snapshot_json?.__filterName].filter(Boolean);
  ...
  const counts=useMemo(()=>{
    ...
    for(const v of vehicles){const s=getCepikStatus(v);c[s]=(c[s]||0)+1;if(hasPriceChange(v))c.price_changed++;}
    return c;
  },[vehicles]);
    ...
    <div className="vdb-tab">
      <div className="vdb-header">
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
          ...
          <button type="button" className="vdb-refresh-btn" onClick={loadVehicles} disabled={loading||refreshing} title="Odśwież">
            <span className={(loading||refreshing)?"vdb-refresh-spin":""}>↻</span>
            {loading?"Ładuję…":refreshing?"Aktualizuję…":"Odśwież"}
          </button>
        </div>
      </div>

      {/* FIX #11: Subtle banner when refreshing existing data */}
      {refreshing && (
        <div className="vdb-refreshing-banner" aria-live="polite">
          <div className="vdb-loading-spin" style={{ width: 14, height: 14, borderWidth: 2 }} />
      ...
      :(
        <div className="vdb-groups">
          {grouped.map(([groupKey,items])=>{
            ...
            // FIX #4: Pass real isJobRunning instead of hardcoded false.
            return <FilterGroup key={groupKey} groupKey={groupKey} items={items} filter={mf}
              collapsed={!!collapsedGroups[groupKey]} onToggle={()=>toggleGroup(groupKey)}
              onOpen={onOpenItem} onDelete={handleDelete} onPatch={handlePatch} onVerify={handleVerify}
              verifyBusy={verifyBusy} pendingDeleteIds={pendingDeleteIds}
              selectedIds={selectedIds} onToggleSelect={handleToggleSelect}
              compareIds={compareIds} onToggleCompare={handleToggleCompare}
              viewMode={viewMode} sortKey={sortByGroup[groupKey]||"newest"} onSortChange={setSortForGroup}
              notes={notes} onSaveNote={setNote} viewedIds={viewedIds} onMarkViewed={markViewed}
              onRunScan={onRunFilter} isJobRunning={isJobRunning}/>;
          })}
        </div>
      )}
